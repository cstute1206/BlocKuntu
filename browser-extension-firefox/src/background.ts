...
const extensionSessionId = `firefox-${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
...
function recordExtensionDiagnostic(
  ...
  void browserIdentity().then((browserName) =>
    queueDiagnostic({
...
function loadDiagnosticQueue(): Promise<void> {
  return browser.storage.local
    .get(DIAGNOSTIC_QUEUE_STORAGE_KEY)
    .then((items) => {
    ...
    .catch((error: unknown) => {
      console.warn(`BlocKuntu failed to load diagnostic queue: ${errorMessage(error)}`);
    });
}

function persistDiagnosticQueue(): Promise<void> {
  return browser.storage.local
    .set({ [DIAGNOSTIC_QUEUE_STORAGE_KEY]: diagnosticQueue })
    .catch((error: unknown) => {
      console.warn(`BlocKuntu failed to persist diagnostic queue: ${errorMessage(error)}`);
    });
}
...
function ensureLifecycleAlarm(name: string): void {
  void browser.alarms
    ...
    .catch((error: unknown) => {
      console.warn(`BlocKuntu failed to create lifecycle alarm ${name}: ${errorMessage(error)}`);
    });
}
